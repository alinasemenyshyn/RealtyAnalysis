...
class Scraper:
    ...
    def scrap_items(self, count_page=5):
        pages = []
        for i in range(count_page):
            try:
                response = requests.get(f'https://dom.ria.com/node/searchEngine/v2/?addMoreRealty=false&excludeSold=1&category=1&realty_type=2&operation=1&state_id=0&city_id=0&in_radius=0&with_newbuilds=0&price_cur=1&wo_dupl=0&complex_inspected=0&sort=inspected_sort&period=0&notFirstFloor=0&notLastFloor=0&with_map=0&photos_count_from=0&with_video_only=0&firstIteraction=false&fromAmp=0&market=3&type=list&client=searchV2&polygonAreaId=0&operation_type=1&page={i}&ch=242_239%2C247_252&mobileStatus=1'
